...
P3a completion update (2026-09-23): additive migration `003_vehicle_identity_archive.js` preserves every existing vehicle/child relation and adds nullable identity/purchase metadata, reversible `archived_at`, and positive `revision=1` without fabricated values or relaxing required chassis. Owner-scoped detail, allowlisted PATCH, idempotent transactional archive/restore, archive-aware lists/reminders and Demo parity are implemented. `VEHICLE_ARCHIVE_ENABLED` defaults off: archive/restore and old-DELETE protection activate together only after compatible instances are drained; explicit filters and reminder exclusion are safe immediately.

P3b implementation update (2026-09-23): the authenticated flat `/vehicle?id=...` detail page, changed-fields-only PATCH form, explicit active/archive list views, reversible archive confirmation, archived detail/restore, Demo activation and dashboard detail links are implemented without a schema or backend contract change. Normal Vehicles UI no longer calls hard DELETE and archive-disabled 409 never falls back to deletion. Automated checks cover routing, invalid/not-found states, filters, PATCH, archive/restore, dialog structure and Demo network isolation. Manual desktop/390px browser review remains required before declaring the phase fully complete. Optional chassis, mileage/service/schedule/obligation, billing and permanent-deletion redesign remain pending.

P4a implementation update (2026-09-25): migration 004 and the backend-only odometer domain foundation are implemented. Kilometre readings use `occurred_on` in Europe/Athens, retain void/replacement evidence, and backfill one undated `legacy_baseline` from each nonnull vehicle cache, including archived vehicles. Existing vehicle scalar writes mediate that baseline transactionally; dated-history activation remains off by default and no public reading endpoint, Demo history model or frontend mileage-history UI is exposed.
...
| --- | --- |
| Database | [001](../backend/migrations/001_initial_schema.js), [002](../backend/migrations/002_align_legacy_schema.js) and additive [003](../backend/migrations/003_vehicle_identity_archive.js) are present. BIGSERIAL keys, snake_case SQL, DATE business dates, TIMESTAMPTZ audit dates, NUMERIC(12,2) money. Do not alter an applied file or its checksum. |
...
| Vehicles | [vehicles.js](../backend/routes/vehicles.js) still requires vehicleType and chassisNumber; chassis_number is NOT NULL and unique per user. P3a adds nullable identity/purchase fields, owner-scoped detail/PATCH and gated reversible archive/restore. current_mileage remains one editable scalar. Legacy DELETE physically deletes with FK cascades only while the archive capability is off; after activation it returns a compatibility conflict. |
...
| odometer_readings — P4a implemented | occurred_on nullable only for legacy baseline, mileage_km, source manual/legacy_baseline/inspection/maintenance, notes, voided_at, void_reason and replaces_reading_id. P4a is kilometres-only and has no confidence or unit column. | Composite vehicle ownership FK CASCADE; deferred replacement FK within the same user/vehicle. Corrections never edit mileage in place. Service linkage remains deferred. | One active dated reading per vehicle/day, one active legacy baseline, one replacement per original; mileage>=0; source/date and void metadata checks; created_at. Neighbor validation and cache changes use locked transactions. |
...
| --- | --- | --- | --- |
| 003_vehicle_identity_archive.js | Implemented additively: nullable registration_plate,registration_country,make,vin,fuel_type,purchase_date,purchase_amount,currency,archived_at; revision defaults to 1; active partial index. The target-model `UNIQUE(id,user_id)` is deferred until the first migration that introduces a composite vehicle ownership FK, because the current schema references the global vehicle primary key only. No guessed identity backfill, archive null for all. | Old writers continue with required chassis_number. Archive-aware list/mutation/cron compatibility is deployed with activation off; drain old code and ship P3b before enabling. | Additive rollback to compatibility release; original code can display archived cars and hard-delete them, so not a supported rollback after activation. Disposable tests validate unchanged legacy IDs/counts/children and zero initially archived vehicles. |
...
- Status: completed 2026-09-23 as a gated schema/API foundation; P3b UI activation remains pending.
...
- Status: implemented 2026-09-23; automated verification complete, manual desktop/390px browser review still required before final completion sign-off.
