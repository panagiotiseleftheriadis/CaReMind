...
| Reminders | [notifications](../backend/routes/notifications.js) uses dates, per-row notification_days, excludes completed rows, caps at 50 and warns at three days. [cron](../backend/routes/cron.js) selects an exact date, lacks completed/inactive filtering and delivery deduplication, sends sequentially with delay. No scheduler definition is checked in. |
...
| Frontend | Flat multipage scripts/globals; shared ui.js and api.js. index.html is login today. Vercel strips .html and rewrites one-segment paths; local static serving differs. Public-page detection is duplicated and cannot assume every new public URL is already exempt. |
...
| Email errors / reminder filtering | Fix before V2 feature work | Stop reporting failed Resend sends as success; exclude inactive/completed cases before commercial reminders. |
