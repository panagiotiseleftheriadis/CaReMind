...
cron.schedule("0 9 * * *", async () => {
  // ΤΡΕΧΕΙ κάθε 30 δευτερόλεπτα για TEST (μετά θα το κάνουμε 1 φορά/μέρα)
...
`);
    ...
    // 🔽 ΕΔΩ ΜΠΑΙΝΕΙ ΤΟ BLOCK ΠΟΥ ΜΟΥ ΕΣΤΕΙΛΕΣ 🔽
    if (rows.length > 0) {
      for (const item of rows) {
        if (!item.email) continue;

        // Μapping: αν στη βάση έχεις "oil", θα το δείχνει "Αλλαγή Λαδιών"
        const typeLabels = {
          oil: "Αλλαγή Λαδιών",
        ...
        // Μορφοποίηση ημερομηνίας σε ΗΗ/ΜΜ/ΥΥΥΥ
