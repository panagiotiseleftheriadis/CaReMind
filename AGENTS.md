...
- `migrations/001_initial_schema.js` creates tables, indexes, constraints and updated_at triggers. `002_align_legacy_schema.js` adds legacy alignment fields/indexes/check constraints. `003_vehicle_identity_archive.js` is the first V2 migration and is additive only. Migration files export `up(connection)`.
