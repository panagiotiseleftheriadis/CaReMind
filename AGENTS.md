...
- Demo schema version 2 mirrors P3a vehicle detail/PATCH/archive/restore/filter/revision semantics. Its archive capability is off after seed/reset and can be enabled explicitly; archive preserves Demo maintenance and costs.
...
- **P3a activation:** schema/API foundation is present, but `VEHICLE_ARCHIVE_ENABLED` defaults off. Before enabling it, drain archive-unaware backend instances and ship P3b so normal UI removal uses reversible archive. While off, omitted vehicle lists include all rows and legacy DELETE remains physical; when on, omitted lists are active-only and legacy DELETE returns 409. Cron and notification projections always exclude archived vehicles. Permanent/full-account deletion remains separate.
