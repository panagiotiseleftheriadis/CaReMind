...
- Pages are `index.html` (public landing), `login.html` (login/password recovery/demo entry), `register.html`, `dashboard.html`, `vehicles.html`, `maintenance.html`, `costs.html`, `account.html` and `admin.html`, with page-specific scripts/styles. Scripts use browser globals and depend on HTML script order; inspect each page before changing shared code.
