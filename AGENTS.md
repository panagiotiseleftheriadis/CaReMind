...
- Pages are `index.html` (public landing), `login.html` (login/password recovery/demo entry), `register.html`, `onboarding.html` (protected post-verification welcome), `dashboard.html`, `vehicles.html`, `vehicle.html` (flat `/vehicle?id=...` detail), `maintenance.html`, `costs.html`, `account.html` and `admin.html`, with page-specific scripts/styles. Scripts use browser globals and depend on HTML script order; inspect each page before changing shared code.
...
- Application tables: companies, users, vehicles, maintenances, costs, interest_requests, refresh_tokens, email_verification_codes, verification_codes, password_reset_codes and notification_recipients. Resource ownership uses `user_id`; routes also check vehicle ownership when attaching maintenance/costs. Migration 003 additively extends vehicles with identity/purchase metadata, reversible `archived_at` and positive `revision`; chassis remains required and per-user unique. Foreign keys still cascade dependent records on legacy vehicle or full-account deletion. Recipients are soft-deactivated.
- `migrations/001_initial_schema.js` creates tables, indexes, constraints and updated_at triggers. `002_align_legacy_schema.js` adds legacy alignment fields/indexes/check constraints. `003_vehicle_identity_archive.js` adds vehicle identity/archive state. P4a migration `004_odometer_readings.js` adds owner-scoped kilometre evidence, correction links and an exact legacy current-mileage baseline without deriving maintenance mileage. Migration files export `up(connection)`.
- `scripts/migrate.js` sorts numbered JS filenames lexicographically, uses one dedicated `pg.Client` for a session advisory lock and all migration queries, runs each pending migration in a transaction and records a SHA-256 checksum in `schema_migrations`. Checksums canonicalize CRLF to LF for cross-platform stability while preserving lone CR and all other content; substantive changes to applied migrations still fail closed. It closes the session in finally. `MIGRATION_DATABASE_URL` must be direct/non-pooled and is required in production/CI/Vercel; only local development can fall back to a loopback `DATABASE_URL`. Known pooler host labels and URL host/database overrides are rejected. Arbitrary proxies cannot be detected: verify the actual endpoint. Migration TLS verifies remote certificates, independently of runtime `DB_SSL`; query URL parameters are limited to `sslmode` and optional `channel_binding=prefer`. Queries use the public schema and a 60-second server statement timeout. It aborts on changed applied files; never change substantive content in an applied migration.
...
- The documented backend deployment uses a separate Vercel project rooted at `backend/`, auto-detecting the exported Express app. There is intentionally no backend `vercel.json` or Edge `middleware.js`; a test asserts this. `vercel-build` runs migrations only for an unambiguous Vercel Production environment, skips them with an explicit message for Preview, and fails closed for missing, conflicting or unsupported environment metadata. Repository configuration does not establish the current state of live hosting, DNS or database migrations.
...
- Demo schema version 2 mirrors vehicle detail/PATCH/archive/restore/filter/revision semantics. Its stored capability is off after seed/reset and is activated browser-only when the P3b Vehicles/Vehicle Detail UI opens; archive preserves Demo maintenance and costs.
...
| Deployment build hook | `npm run vercel-build` applies database migrations for Vercel Production, skips them for Preview, and rejects an unknown/conflicting target; Production still requires direct `MIGRATION_DATABASE_URL` |
...
- **P3 archive activation:** P3a schema/API and the P3b detail/archive UI are present, but `VEHICLE_ARCHIVE_ENABLED` defaults off. Before enabling it, deploy the compatible UI/backend everywhere, drain archive-unaware instances and verify migration 003 on the target. While off, omitted vehicle lists include all rows and legacy DELETE remains physical; the P3b UI uses explicit filters and never calls/falls back to DELETE. When on, omitted lists are active-only and legacy DELETE returns 409. Cron and notification projections always exclude archived vehicles. Permanent/full-account deletion remains separate. Once archived rows exist, disabling archive-aware behavior or rolling back to archive-unaware binaries is unsafe.
- **P4a odometer foundation:** `odometer_readings` is kilometre-only evidence with Athens calendar dates, retained corrections and an undated `legacy_baseline`; `vehicles.current_mileage` remains its compatibility cache. Existing scalar vehicle writes are mediated transactionally. Once dated evidence exists they may repeat the derived value but cannot change or clear it. `ODOMETER_HISTORY_ENABLED` defaults off and no public reading endpoints or frontend history UI exist yet.
