...
- **Coverage:** real PostgreSQL migration integration coverage exists through `test:postgres`; there is still no full browser E2E suite or real-database API integration suite. API regression tests cover JWT purpose separation, algorithm restrictions and intended reset/account-change token flows using database stubs.
